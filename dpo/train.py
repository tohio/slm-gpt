...
def resolve_model_config(checkpoint_path: str, args: DPOArgs, device: str) -> Tuple[ModelConfig, dict]:
    ...
    if not os.path.exists(resolved_path):
        fallback = os.path.join(os.path.dirname(checkpoint_path), "sft_final.pt")
        if os.path.exists(fallback):
            print(f"[Notice] '{resolved_path}' not found. Falling back to '{fallback}'.")
            resolved_path = fallback

    assert os.path.exists(resolved_path), f"SFT checkpoint not found at '{checkpoint_path}' or fallback '{fallback}'."
...
def train_dpo(args: DPOArgs):
    ...
    assert os.path.exists(args.data_path), f"Preference dataset not found at: '{args.data_path}'"
    ...
    for epoch in range(args.epochs):
        for step_idx, batch in enumerate(loader):
            ...
            with torch.amp.autocast(device_type="cuda", dtype=dtype, enabled=use_autocast):
                with torch.no_grad():
                    all_ref_logits = ref_model(all_ids)[0]
                ...
                all_pi_logits = policy_model(all_ids)[0]
